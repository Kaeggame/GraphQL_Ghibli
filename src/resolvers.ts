...
const resolvers = {
  Query: {
    async getFilms() {
      ...
      return films.map(film => ({
        ...film,
        people: film.people.filter(url => url.includes('people')).map(url => ({ __typename: 'People', id: url.split('/').pop(), eyeColor: 'N/A' }))
      }));
    },
    async getPeople() {
  ...
  Film: {
    people(parent) {
      return parent.people.map(async ({ id }) => {
        return fetch(`https://ghibliapi.dev/people/${id}`).then(res => res.json());
      });
    }
  },
  People: {
